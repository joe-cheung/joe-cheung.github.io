...
What is in that missing >0.4? _Everything!_ While random babbling sufficed at the start, nothing short of true understanding will suffice for ideal prediction. _The last bits are deepest. _Analogous to humans: we all perform everyday actions like buttoning our shirts well given enough practice and feedback, but we differ when we run into the long tail of choices that are:
...
Scaling just works: just train a 1.2b-param Transformer on half a thousand different tasks and the scaling curve looks exactly like you'd expect:
